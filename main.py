...
def all_crea(search):
    #site="https://www.google.com/+search+?q=%E6%B5%81%E6%84%9F&tbs=qdr:y19"
    db = pymysql.connect("203.64.84.94", "Fish", "851217", "project105")
    #db = pymysql.connect("localhost", "root", "97462835", "project105")
    cur = db.cursor()
    site = "https://www.google.com/search?q="+search+"&tbs=qdr:y19"
    count=0
    a = "天前"
    b = "小時前"
    c = "分鐘前"
                ...
                if st3 != None:
                    if su == -1:
                        ...
                        if i!=None:
                            ...
                            tmm=0
                            y=0
                            sql2 = "INSERT INTO disease_t (name,readable,rich,timely,url,date) VALUES ('%s','%f','%f','%f','%s','%s')"% (st3.text,0 , 0, y, st8, i)
                            print (sql2)
                            try:
                                cur.execute(sql2)
                                db.commit()
